...
def create_recipe(recipe: RecipeCreate):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO recipes (name, description, ingredients, instructions, cuisine, difficulty, category_id) VALUES (?, ?, ?, ?, ?, ?, ?)", (recipe.name, recipe.description, recipe.ingredients, recipe.instructions, recipe.cuisine, recipe.difficulty, recipe.category_id)

    )
    conn.commit()
    recipe_id = cursor.lastrowid
    conn.close()
    return Recipe(id=recipe_id, name=recipe.name, description=recipe.description, ingredients=recipe.ingredients, instructions=recipe.instructions, cuisine=recipe.cuisine, difficulty=recipe.difficulty, category_id=recipe.category_id)
...
def update_recipe(recipe_id: int, recipe: RecipeCreate):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE recipes set name= ?, description=?, ingredients=?, cuisine=?, difficulty=?, category_id=? WHERE id = ?",
