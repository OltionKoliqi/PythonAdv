...
def create_recipe(recipe_name, description, ingredients, instructions, cuisine, difficulty, category_id):
    response = requests.post(f"{API_BASE_URL}/recipe/", json={
        "name": recipe_name,
...
def update_recipe(recipe_id, recipe_name, description, ingredients, instructions, cuisine, difficulty, category_id):
    response = requests.put(f"{API_BASE_URL}/recipe/{recipe_id}", json={
        "name": recipe_name,
        "description": description,
        "ingredients": ingredients,
        "instructions": instructions,
...
def delete_recipe(recipe_id):
    response = requests.delete(f"{API_BASE_URL}/recipe/{recipe_id}")
    return response.json()
...
if selected_menu == "Dashboard":
    ...
    recipe_instruction = st.text_area("Instructions", key="new_recipe_instructions")
    ...
    if st.button("Add Recipe", key="add_recipe_button"):
        if all([recipe_name, recipe_description, recipe_ingredients, recipe_instruction, recipe_cuisine, recipe_difficulty, selected_category_id is not None]):
            create_recipe([recipe_name, recipe_description, recipe_ingredients, recipe_instruction, recipe_cuisine, recipe_difficulty, selected_category_id])
            st.success(f"Recipe '{recipe_name}' added succesfully")
        else:
            st.error("All fields must be filled and category selected to add to recipe.")
    ...
    if recipe_list:
        ...
        if manage_action == "Edit":
            ...
            if recipe_to_edit:
                ...
                if category_list:
                    category_name = [cat['name'] for cat in category_list]
                    edit_category_name = st.selectbox("Category", category_name, index=category_name.index(next(cat['name'] for cat in category_list if cat['id'] == selected_recipe['category_id'])), key="edit_recipe_category")
                ...
                if st.button("Update Recipe", key="update_recipe_button"):
                    if all([edit_name, edit_description,  edit_ingredients,  edit_instructions, edit_cuisine, edit_difficulty, edit_category_id is not None]):
                        ...
                        st.success(f"Recipe '{edit_name}' updated succesfully!")
                    else:
                        st.error("All fields must be fille and category selected to update recipe.")
